...
% list = readmatrix('wav_920.xlsx');
list = readmatrix('wav_650_900.xlsx');

combos = nchoosek(1:length(list),3);
%%

for l = 1:length(combos)
    A{l} = [list(combos(l,1),2) list(combos(l,1),3) list(combos(l,1),4); list(combos(l,2),2) list(combos(l,2),3) list(combos(l,2),4); list(combos(l,3),2) list(combos(l,3),3) list(combos(l,3),4)];
end
...
% wav1_plot = master_list_sorted(1:length(combos)-30,3);
% wav2_plot = master_list_sorted(1:length(combos)-30,4);
% wav3_plot = master_list_sorted(1:length(combos)-30,5);
% wavelengths_plot = [wav1_plot,wav2_plot,wav3_plot];
...
ax = gca;
ax.YColor = 'blue';
leftcolors = [0 0.4470 0.7410; 0.8500 0.3250 0.0980; 0.9290 0.6940 0.1250];
...
%xlim([0 200000])
%ylim([650 1000])
title({'Graph showing different three-wavelength combinations and their corresponding condition number','alongside extinction coefficients of chromophores'},'FontSize',16)
legend({'Wavelength 1', 'Wavelength 2', 'Wavelength 3', 'HbO', 'HbR', 'oxCCO'},'Location','southwest','FontSize',16)
...
%xlim([0 200000])
%ylim([650 1000])
title({'Graph showing different three-wavelength combinations and their corresponding condition number','alongside the slope of the extinction coefficients of chromophores'},'FontSize',16)
legend({'Wavelength 1', 'Wavelength 2', 'Wavelength 3', 'HbO', 'HbR', 'oxCCO'},'Location','southwest','FontSize',16)
...
rightcolors = [0.4660 0.6740 0.1880; 0.3010 0.7450 0.9330; 0.6350 0.0780 0.1840];
colororder(rightcolors)
ylabel('Sum of Slope of Extinction Coefficients')

%'LineStyle', 'none' ,'marker','.', 'MarkerSize',30);
%xlim([0 200000])
%ylim([650 1000])
title({'Graph showing different three-wavelength combinations and their corresponding condition number','alongside the sum of the slope of the extinction coefficients of chromophores'},'FontSize',16)
legend({'Wavelength 1', 'Wavelength 2', 'Wavelength 3', 'Sum of Slopes', 'HbR', 'oxCCO'},'Location','southwest','FontSize',16)
