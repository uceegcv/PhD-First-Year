...
full_lipid = list(:,10);
% full_HbR = full_HbR/100;
% full_HbO = full_HbO/100;
% full_CCO = full_CCO/10;

absorption_HbR = (full_HbR*0.000024)/2.303;
absorption_HbO = (full_HbO*0.000056)/2.303;
absorption_CCO = (full_CCO*0.000003)/2.303;
...
%     mua_brain(f) = (full_HbO(f)*conc_HbO)/10 + (full_HbR(f)*conc_HbR)/10 + absorption_water(f)*W + absorption_lipid(f)*L + B;
%     k = k+1;
% end
